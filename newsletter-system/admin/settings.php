...
<form method="post" class="field-stack">
    ...
    <div class="panel">
        ...
        <div class="row g-3">
            <div class="col-md-4"><label>Sender name <input class="form-control" name="sender_name" value="<?= h(setting('sender_name', 'Newsletter')) ?>"></label></div>
            <div class="col-md-4"><label>Sender email <input class="form-control" type="email" name="sender_email" value="<?= h(setting('sender_email', '')) ?>"></label></div>
            <div class="col-md-4"><label>Reply-to email <input class="form-control" type="email" name="reply_to" value="<?= h(setting('reply_to', '')) ?>"></label></div>
            <div class="col-md-4"><label>SMTP host <input class="form-control" name="smtp_host" value="<?= h($smtp['host'] ?? '') ?>"></label></div>
            <div class="col-md-2"><label>Port <input class="form-control" type="number" name="smtp_port" value="<?= h($smtp['port'] ?? 587) ?>"></label></div>
            <div class="col-md-3"><label>Username <input class="form-control" name="smtp_username" value="<?= h($smtp['username'] ?? '') ?>"></label></div>
            <div class="col-md-3"><label>Password <input class="form-control" type="password" name="smtp_password" placeholder="Leave blank to keep saved"></label></div>
            <div class="col-md-3"><label>Encryption <select class="form-select" name="smtp_encryption"><option value="tls">TLS</option><option value="ssl" <?= ($smtp['encryption'] ?? '') === 'ssl' ? 'selected' : '' ?>>SSL</option><option value="">None</option></select></label></div>
            <div class="col-md-3"><label>Batch size <input class="form-control" type="number" name="batch_size" value="<?= h($smtp['batch_size'] ?? 50) ?>"></label></div>
            <div class="col-md-3"><label>Delay seconds <input class="form-control" type="number" name="batch_delay_seconds" value="<?= h($smtp['batch_delay_seconds'] ?? 60) ?>"></label></div>
        </div>
    </div>
    <div class="panel">
